...
export default function AdminDealers() {
  ...
  const handleToggleStatus = async (user: any) => {
    try {
      ...
      await adminApi.put(`/users/${user.id}/status`, newStatus)
  ...
  const columns = [
    ...
    { title: '状态', dataIndex: 'status', width: 80, render: (v: string) => <Tag color={v === 'active' ? 'green' : 'red'}>{v === 'active' ? '正常' : '禁用'}</Tag> },
  ...
  return (
    <div>
      ...
      <Modal title={`${selectedDealer?.name} - 账号列表`} open={userVisible} onCancel={() => setUserVisible(false)} footer={null} width={700}>
        <Table
          columns={userColumns}
